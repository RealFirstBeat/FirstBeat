...
class PlaylistSwipeServiceTest extends DummyObject {
	...
	void testLikeTrack_Success() {
		...
		// When: likeTrack 호출
		String result = playlistSwipeService.likeTrack(mockUser, Long.valueOf(spotifyTrackId));
	...
	void testLikeTrack_TrackAlreadyExists() {
		...
		BusinessException exception = assertThrows(BusinessException.class, () ->
			playlistSwipeService.likeTrack(mockUser, Long.valueOf(spotifyTrackId))
		);
	...
	void testSkipTrack_Success() {
		...
		when(recommendationService.removeTrackFromRecommendations(mockUser().getId(), 12345L)).thenReturn(true);

		// When: skipTrack 메서드 호출
		String result = playlistSwipeService.skipTrack(mockUser(), 12345L);

		// Then: 결과 검증 및 Mock 호출 확인
		assertEquals("추천 트랙이 성공적으로 스킵되었습니다.", result);
		verify(recommendationService, times(1)).removeTrackFromRecommendations(mockUser().getId(), 12345L);
	}
	...
	void testSkipTrack_TrackNotInRecommendations() {
		...
		// When & Then
		BusinessException exception = assertThrows(BusinessException.class, () ->
			playlistSwipeService.skipTrack(mockUser, 67890L) // 다른 트랙 ID
		);
		...
		verify(recommendationService, never()).removeTrackFromRecommendations(anyLong(), anyLong());
	}
}