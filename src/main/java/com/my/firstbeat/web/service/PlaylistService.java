...
public class PlaylistService {
	...
	public Playlist getDefaultPlaylist(Long userId) {
		return playlistRepository.findByUserDefaultPlaylist(userId)
			.orElseThrow(() -> new BusinessException(ErrorCode.PLAYLIST_NOT_FOUND));
	}

    public PlaylistsData searchPlaylists(String query, int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<Playlist> playlistPage = playlistRepository.findByTitleContaining(query, pageable);
